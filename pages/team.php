...
<table id="teams" class="table table-bordered">
	<thead>
		<tr>
			<th class="col-md-2 text-center">FUNCIONÁRIO</th>
			<th class="col-md-2 text-center">MEMBRO/s</th>
	...
	<tbody>
		...
		<?php } ?>
				...
				<td><center>
					<?php
					if($row['pio'] == '1'){
						echo '<a href="../forms/update_forms.php?action=team_stats&io=0&id='.$row['tid'].'" class="btn btn-sm btn-success">Ativ</a>';
					}else{
						echo '<a href="../forms/update_forms.php?action=team_stats&io=1&id='.$row['tid'].'" class="btn btn-sm btn-danger">Inativo</a>';
					}	
					 ?>
					</center>
				</td>
				<td><center><a type="button" onclick="update_team(<?php echo $row['tid']; ?>)"><i class="fa fa-edit"></i> Alterar</a></center></td>
			</tr>
