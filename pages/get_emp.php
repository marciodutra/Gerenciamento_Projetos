<form method="POST" id="att_form" >
   ...
   if($_GET['tid'] == 'all'){ 
      $query=  mysqli_query($conn, "SELECT *,CONCAT(lastname,', ',firstname,' ',midname) as name FROM employee  where io = '1' order by name ");
        ...
        <th><center><label><input type="checkbox"  id="in<?php echo $id4 ?>"  onclick="att_in(<?php echo $id4 ?>)" class="checkitems1" value="<?php echo $id4 ?>" <?php echo $check1 ?>> DENTRO</label></center></th>
        <th><center><label><input type="checkbox"  id="out<?php echo $id4 ?>"  onclick="att_out(<?php echo $id4 ?>)" class="checkitems2" value="<?php echo $id4 ?>" <?php echo $check2 ?>> FORA</label></center></th>
      </tr>
    ...
    ?>
      <tr>
        ...
        if( mysqli_num_rows($query2) > 0){ ?>
         ...
         <td><center><label><input type="checkbox" id="in<?php echo $id ?>" onclick="att_in(<?php echo $id ?>)" class="checkitems1" value="<?php echo $id ?>" <?php   if($row2['time_in'] != '00:00:00'){ echo 'checked';} ?>> DENTRO</label></center></td>
        <td><center><label><input type="checkbox" id="out<?php echo $id ?>" onclick="att_out(<?php echo $id ?>)" class="checkitems2" value="<?php echo $id ?>" <?php if($row2['time_out'] != '00:00:00'){ echo 'checked';} ?>> FORA</label></center></td>


       <?php } else{ ?>
        <td><center><label><input type="checkbox" id="in<?php echo $id ?>" onclick="att_in(<?php echo $id ?>)" class="checkitems1" value="<?php echo $id ?>" > DENTRO</label></center></td>
        <td><center><label><input type="checkbox" id="out<?php echo $id ?>" onclick="att_out(<?php echo $id ?>)" class="checkitems2" value="<?php echo $id ?>" > FORA</label></center></td>
        <?php } ?>
      </tr>
  ...
  <div class="col-md-12" id="act">
   <center><a href="index.php?page=attendance&tid=all" class="btn btn-info btn-sm">Cancelar</a>
  </div></center><br>
</form>
<script>
